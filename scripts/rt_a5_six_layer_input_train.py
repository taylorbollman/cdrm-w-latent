#!/usr/bin/env python3
"""Fresh six-layer input bypass: constant coefficient 0.01, initial 10k diagnostic.
...
from scripts.rt_a5_six_layer_input import COEFFICIENT, build_model
...
SCHEMA = "rt-a5-six-layer-input-training-v1"
VARIANT_ARCHITECTURES = {"input": "rt"}
...
def source_manifest():
    ...
    additions = [ROOT / "scripts" / name for name in
                 ("rt_a5_six_layer_input.py", "rt_a5_six_layer_input_train.py")]
    additions += list((ROOT / "configs/rt_a5_six_layer_input").glob("*.json"))
...
def validate_coefficient_state(model):
    if (model.backbone.injection_coefficient != COEFFICIENT
            or model.backbone.injection_variant != "input"
            or model.backbone.config.n_layers != 6):
        raise ValueError("Model must retain the six-layer fixed input coefficient 0.01")
...
def save_checkpoint(path, *, model, optimizer, contract, completed, order_chain,
    ...
    validate_coefficient_state(model)
    packet = {
        ...
        "injection_coefficient": COEFFICIENT, "coefficient_learned": False,
    }
    ...
    return {"path": str(path.resolve()), "sha256": file_sha256(path),
            ...
            "examples_seen": packet["examples_seen"], "injection_coefficient": COEFFICIENT}
...
def load_checkpoint(path, *, model, optimizer, contract):
    ...
    if packet.get("schema") != SCHEMA or packet.get("contract") != contract:
        raise ValueError("Six-layer input checkpoint source/data/model/runtime/objective contract differs")
    ...
    if (packet.get("coefficient_learned") is not False
            or packet.get("injection_coefficient") != COEFFICIENT):
        raise ValueError("Checkpoint fixed input coefficient differs")
    if packet.get("initialization") != json_value(model.nextlat_initialization):
        raise ValueError("Six-layer input initialization differs")
...
def run(args):
    if args.variant not in VARIANT_ARCHITECTURES:
        raise ValueError("Unsupported Six-layer input variant")
    ...
    report = {
        ...
        "checkpoint_selection": "initial 10k diagnostic; fixed injection coefficient 0.01; user may stop after completed updates; confirmation not evaluated",
        "injection_coefficient": COEFFICIENT, "coefficient_learned": False,
    ...
    try:
        ...
        for update in range(completed + 1, args.updates + 1):
            ...
            if update % args.eval_every == 0 or update in checkpoint_steps or stopping is not None:
                ...
                for role, (dx, dy) in dev.items():
                    ...
                    tracker.log({"update": update, "injection/coefficient": current_coefficient,
        ...
        report.update(status="complete" if reached_endpoint else "stopped",
                      ...
                      injection_coefficient=COEFFICIENT,
        ...
        tracker.summary({"completed_updates": completed, "requested_endpoint": args.updates,
                         ...
                         "injection_coefficient": COEFFICIENT, "confirmation_evaluated": False,
