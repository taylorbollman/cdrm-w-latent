#!/usr/bin/env python3
"""Native-checkpoint comparison and capacity of bounded-workspace RT backward."""
...
SOURCES=tuple(sorted(set(F3_SOURCES)|{"scripts/olmo_f3d_validate.py","cdrm/pretrained/olmo_rt_kernels.py","cdrm/pretrained/olmo_rt_backward_kernels.py","cdrm/pretrained/olmo_rt_memory.py","cdrm/pretrained/olmo_rt_recompute_kernels.py"}))
PROTOCOL=ROOT/"docs/reports/olmo1b-f3d/protocol.md"
...
def count_backward_tiles():
    ...
    original=olmo_rt_recompute_kernels.backward_recomputed_tile
    observed={"count":0}
    def counted(*args,**kwargs):
        observed["count"]+=1
        return original(*args,**kwargs)
    with patch.object(olmo_rt_recompute_kernels,"backward_recomputed_tile",counted):
        yield observed
...
def new_plan(model,batch,mode,variant):
    ...
    base.backward_memory="recompute" if variant=="recompute" else "materialized"
...
def compare_variant(model,batch,mode,variant):
    ...
    expected_calls=expected_fused_backward_calls(batch,mode) if variant=="recompute" else 0
    dispatch_matches=reference_calls["count"]==0 and candidate_calls["count"]==expected_calls
    # Only backward memory/reconstruction changes; both arms retain validated F3b
...
def main(argv=None):
    ...
    parser.add_argument("--length",type=int,choices=(32,512,1024,2048),default=32)
    ...
    report={"schema":"olmo-f3d-native-v1","status":"running","stage":args.stage,
    ...
    tracker=OnlineTracker(project="pretrained-fbt-rt-nextlat",output_dir=args.output_dir,
        group="olmo1b-f3d-rt-memory",name="olmo-"+args.output_dir.name)
    def publish(check):
