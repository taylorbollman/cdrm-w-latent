#!/usr/bin/env python3
"""Fresh four-layer input bypass: linear warmup, initial 10k diagnostic.
...
from scripts.rt_a5_linear_input import SCHEDULE, build_model, coefficient, set_update
...
SCHEMA = "rt-a5-linear-input-training-v1"
...
def source_manifest():
    ...
    additions = [ROOT / "scripts" / name for name in
                 ("rt_a5_linear_input.py", "rt_a5_linear_input_train.py")]
    additions += list((ROOT / "configs/rt_a5_linear_input").glob("*.json"))
...
def validate_schedule_state(model, completed):
    if (model.schedule_update != completed
            or model.backbone.injection_coefficient != coefficient(completed)):
...
def save_checkpoint(path, *, model, optimizer, contract, completed, order_chain,
    ...
    packet = {
        ...
        "schedule": json_value(SCHEDULE), "schedule_update": completed,
        "injection_coefficient": coefficient(completed),
    }
    ...
    return {"path": str(path.resolve()), "sha256": file_sha256(path),
            ...
            "examples_seen": packet["examples_seen"], "injection_coefficient": coefficient(completed)}
...
def load_checkpoint(path, *, model, optimizer, contract):
    ...
    if packet.get("schema") != SCHEMA or packet.get("contract") != contract:
        raise ValueError("Linear-input checkpoint source/data/model/runtime/objective contract differs")
    ...
    if (packet.get("schedule") != SCHEDULE or packet.get("schedule_update") != completed
            or packet.get("injection_coefficient") != coefficient(completed)):
        raise ValueError("Checkpoint linear schedule or current coefficient differs")
    if packet.get("initialization") != json_value(model.nextlat_initialization):
        raise ValueError("Linear-input initialization differs")
...
def make_contract(args, *, model, sources, data_root, train_x, hardware):
    ...
    contract.update(
        ...
        injection_schedule=json_value(SCHEDULE),
...
def run(args):
    if args.variant not in VARIANT_ARCHITECTURES:
        raise ValueError("Unsupported Linear-input variant")
    ...
    report = {
        ...
        "checkpoint_selection": "initial 10k diagnostic; linear warmup spans 50k; user may stop after completed updates; confirmation not evaluated",
        "schedule": json_value(SCHEDULE), "injection_coefficient": coefficient(completed),
    ...
    try:
        ...
        tracker.summary({"completed_updates": completed, "requested_endpoint": args.updates,
                         ...
                         "injection_coefficient": coefficient(completed), "confirmation_evaluated": False,
