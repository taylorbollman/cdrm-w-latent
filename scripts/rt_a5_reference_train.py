#!/usr/bin/env python3
"""Author-style GPT architecture trained with our unchanged A5 update/evaluator."""
...
SCHEMA = "rt-a5-reference-gpt-training-v1"
ARCHITECTURE = "reference_gpt"
...
def source_manifest():
    ...
    additions = [ROOT / "scripts" / name for name in
                 ("rt_a5_reference_gpt.py", "rt_a5_reference_train.py")]
    additions += list((ROOT / "configs/rt_a5_reference").glob("*.json"))
...
def load_checkpoint(path, *, model, optimizer, contract):
    ...
    if packet.get("schema") != SCHEMA or packet.get("contract") != contract:
        raise ValueError("Reference GPT checkpoint source/data/model/runtime contract differs")
    ...
    if packet.get("initialization") != json_value(model.a5_initialization):
        raise ValueError("Reference GPT initialization differs")
...
def make_contract(args, *, model, sources, data_root, train_x, hardware):
    return {
        ...
        "architecture_comparison": "author GPT architecture, our unchanged data/loss/update/evaluation",
    }
...
def run(args):
    ...
    model = build_reference_gpt(width=args.width, seed=args.seed, device="cuda")
    ...
    tracker = OnlineTracker(
        ...
        name=args.wandb_run_name or f"reference-gpt-d{args.width}-seed{args.seed}",
...
def parser():
    ...
    p.add_argument("--checkpoint-steps", type=int, nargs="+", default=[10000, 25000, 50000, 100000])
