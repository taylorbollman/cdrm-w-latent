#!/usr/bin/env python3
"""Fresh four-layer input bypass: conservative quadratic warmup, initial 10k diagnostic.
...
SCHEMA = "rt-a5-conservative-input-training-v1"
...
def source_manifest():
    ...
    additions = [ROOT / "scripts" / name for name in
                 ("rt_a5_conservative_input.py", "rt_a5_conservative_input_train.py")]
    additions += list((ROOT / "configs/rt_a5_conservative_input").glob("*.json"))
...
def load_checkpoint(path, *, model, optimizer, contract):
    ...
    if packet.get("schema") != SCHEMA or packet.get("contract") != contract:
        raise ValueError("Conservative-input checkpoint source/data/model/runtime/objective contract differs")
    ...
    if (packet.get("schedule") != SCHEDULE or packet.get("schedule_update") != completed
        ...
        raise ValueError("Checkpoint conservative schedule or current coefficient differs")
    if packet.get("initialization") != json_value(model.nextlat_initialization):
        raise ValueError("Conservative-input initialization differs")
...
def run(args):
    if args.variant not in VARIANT_ARCHITECTURES:
        raise ValueError("Unsupported Conservative-input variant")
    ...
    report = {
        ...
        "checkpoint_selection": "initial 10k diagnostic; conservative quadratic warmup spans50k; user may stop after completed updates; confirmation not evaluated",
