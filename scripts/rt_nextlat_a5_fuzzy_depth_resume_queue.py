#!/usr/bin/env python3
"""Host supervisor for an interrupted three-layer mixed-task pilot resumed to total 5k."""
...
class Queue(OriginalQueue):
    def status(self, phase, **values):
        packet = {"schema": "rt-nextlat-mixed-depth-resume-queue-v1", "phase": phase,
    ...
    def run(self):
        ...
        if (report["status"] not in ("complete", "stopped")
                ...
                or report["requested_endpoint"] != 5000 or not self.config["resume_update"] <= endpoint <= 5000
            ...
            raise RuntimeError("Resumed mixed run did not obey the authorized 5k/B2560 contract")
        report_args = []
        if endpoint > 0:
            self.invoke("compare", ["python", "-m", "scripts.rt_nextlat_a5_fuzzy_depth_resume_report",
