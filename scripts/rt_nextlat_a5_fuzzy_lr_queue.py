#!/usr/bin/env python3
"""Host supervisor for one matched 5k mixed-task learning-rate pilot."""
...
class Queue(OriginalQueue):
    def status(self, phase, **values):
        packet = {"schema": "rt-nextlat-mixed-lr-queue-v1", "phase": phase,
    ...
    def verify(self):
        ...
        if ready.get("passed") is not True:
            raise RuntimeError("Learning-rate pilot readiness has not passed")
    ...
    def run(self):
        ...
        if (self.runtime / "STOP").exists():
            ...
            return
        self.invoke("train", ["python", "-u", "-m", "scripts.rt_nextlat_a5_fuzzy_lr_train",
        ...
        report_args = []
        if endpoint > 0:
            self.invoke("compare", ["python", "-m", "scripts.rt_nextlat_a5_fuzzy_lr_report",
            ...
            if read(ROOT / self.config["report"] / "report.json").get("status") != "complete":
                raise RuntimeError("Learning-rate comparison did not complete")
