#!/usr/bin/env python3
"""Retain explicit ordinary-efficiency evidence without uploading model weights."""
...
SCHEMA = "olmo-ordinary-efficiency-retention-v1"
MAX_BYTES = 64 * 1024**2
...
REQUIRED_DOCS = ("protocol.md", "summary.json", "results.md", "usage.md",
                 "runtime-test-results.txt", "harness-test-results.txt")
PROJECT_FILES = (
    "scripts/olmo_ordinary_efficiency_report.py", "scripts/olmo_ordinary_efficiency_retain.py",
    "scripts/olmo_ordinary_efficiency_plot.py",
    ...
    "tests/test_olmo_ordinary_execution.py", "tests/test_olmo_ordinary_efficiency.py",
    "tests/test_olmo_ordinary_efficiency_report.py", "tests/test_olmo_ordinary_efficiency_retain.py",
...
def collect_evidence(root=ROOT):
    ...
    require(summary.get("schema") == "olmo-ordinary-efficiency-summary-v1"
            and summary.get("status") == "completed" and summary.get("runs"),
            "Require a completed explicit ordinary-efficiency selection")
    ...
    for selected in summary["runs"]:
        ...
        sources = raw.get("dependencies", {}).get("fa4_sources", {})
        dependencies[name] = len(sources)
        for source in sources:
            relative = "dependency-snapshot/flash_attn/cute/" + source
