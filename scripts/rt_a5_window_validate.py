"""Bounded FP32 correctness and actual-shape smoke for the Mitchell positional/window A5 pilots.
...
from scripts.rt_a5_window_train import source_manifest, train_step
...
def check_small(length, *, position_encoding, second_layer_window):
    x, y = fixture(2, length, 1254 + length)
    packets, checks = {}, {}
    for backend in (("independent", "naive", "tiled") if second_layer_window else ("naive", "tiled")):
        model = build_window_model(width=128, backend="naive" if backend == "independent" else backend,
                                   device="cuda", position_encoding=position_encoding,
                                   second_layer_window=second_layer_window)
        if backend == "independent":
            from scripts.rt_a5_window_reference import direct_window2
            model.backbone.transformer.blocks[1]._real_forward = MethodType(
                direct_window2, model.backbone.transformer.blocks[1])
        ...
        del model, result
    reference_name = "independent" if second_layer_window else "naive"
    a, b = packets[reference_name], packets["tiled"]
...
def actual_shape(tracker, *, position_encoding, second_layer_window):
    x, y = fixture(1024, 12, 1264)
    model = build_window_model(device="cuda", position_encoding=position_encoding,
                               second_layer_window=second_layer_window)
...
def main():
    ...
    p.add_argument("--position-encoding", choices=("alibi", "sinusoidal"), default="sinusoidal")
    p.add_argument("--second-layer-window", type=int, choices=(2,))
    args = p.parse_args()
    ...
    report = {"schema": "rt-a5-window-validation-v1", "status": "running",
    ...
    tracker = OnlineTracker(project="rt-a5-state-tracking", output_dir=out,
                            group=args.wandb_group, name=f"mitchell-{args.position_encoding}-window{args.second_layer_window}-preflight",
                            preserve_state=preserve_rng)
    try:
        tracker.start({"scope": report["scope"], **runtime})
        report["small"] = {}
        model_args = dict(position_encoding=args.position_encoding, second_layer_window=args.second_layer_window)
        report["model_arguments"] = model_args
        for length in ((1, 2, 12, 36) if args.second_layer_window else (12, 36)):
            # NextLat needs a transition, so T1 is covered by the CPU block oracle.
            if length == 1:
                continue
            result = check_small(length, **model_args)
        ...
        report["actual_shape"] = actual_shape(tracker, **model_args)
