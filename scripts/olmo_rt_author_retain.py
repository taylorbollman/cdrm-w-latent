#!/usr/bin/env python3
"""Retain explicitly selected author/native block evidence, without model weights."""
...
SCHEMA = "olmo-rt-author-retention-v1"
MAX_BYTES = 64 * 1024**2
ARTIFACT = ".runtime/olmo1b-step60000/artifacts/artifact-manifest.json"
RECEIPT = "docs/reports/olmo1b-o1/storage-receipt.json"
DOC_FILES = ("protocol.md", "author-port-audit.md", "summary.json", "test-results.txt",
    "retention-test-results.txt",
    "results.md", "assessment.md", "usage.md", "numerical-assessment.md", "operator-audit.md", "execution-notes.md",
    "throughput.png", "throughput.pdf", "native-bridge.png", "native-bridge.pdf",
    "profile-audit.md", "integration-followup.md")
PROJECT_FILES = (
    "scripts/olmo_rt_author_report.py", "scripts/olmo_rt_author_retain.py",
    ...
    "docker/requirements-docker.txt", "tests/test_olmo_rt_author_evidence.py",
    "tests/test_olmo_author.py", "tests/test_rt_block_resources.py", "tests/test_olmo_rt_author_compare.py",
...
def queue_artifacts(runtime):
    ...
    for path in sorted(runtime.iterdir()):
        if path.name != "run_capacity_queue.py" and re.fullmatch(
                r"capacity-[A-Za-z0-9][A-Za-z0-9_-]*\.(?:json|log)", path.name) is None:
            continue
...
def collect_evidence(root=ROOT):
    ...
    for selected in summary["runs"]:
        ...
        if "profile" in selected:
            require(selected["profile"] == row.get("profile"), "Summary profile differs from report")
        ...
        add(path.parent / "author-port-audit.md", prefix + "author-port-audit.md")
        ...
        if row.get("profile"):
            add(path.parent / row["profile"]["trace_file"], prefix + row["profile"]["trace_file"])
        rows.append(row)
        raws.append(raw)
    ...
    preflight = report.event_preflight(root, required=any(raw["status"] == "passed"
        and raw["configuration"].get("region_events") for raw in raws))
    require(summary.get("event_preflight") == preflight, "Selected event preflight changed")
    if preflight:
        add(root / preflight["path"], "runtime/event-preflight.json")
        add(runtime / "event-preflight.log", "logs/event-preflight.log")
    for required in ("protocol.md", "author-port-audit.md", "summary.json", "test-results.txt"):
    ...
    require(sum(path.stat().st_size for name, path in members.items()
        if not name.endswith("/operator-trace.json.gz")) < MAX_BYTES, "Non-trace evidence exceeds 64 MiB")
    return summary, checkpoint, members, {
        ...
        "profiles": {row["name"]: row["profile"] for row in rows if row.get("profile")},
...
def main(argv=None):
    ...
    inventory = build_evidence_archive(archive, [(path, name) for name, path in sorted(members.items())],
        ...
        "are uploaded. Exact declared compressed traces and standalone event-preflight evidence are retained. "
