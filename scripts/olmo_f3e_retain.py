#!/usr/bin/env python3
"""Retain bounded F3e evidence with exact per-run historical source snapshots."""
...
SCHEMA = "olmo-f3e-retention-v1"
PREFIX_ROOT = "gs://fast-chunks/cdrm-w-latent/fbt-rt-nextlat/olmo1b-f3e-multi-rt/"
NATIVE = "olmo-f3e-native-v1"
...
EXTRA_PROJECT_FILES = {
    ...
    "scripts/olmo_f3e_retain.py", "scripts/olmo_f3e_report.py", "scripts/olmo_f3b_retain.py", "scripts/olmo_f3_retain.py", "scripts/olmo_f2_retain.py",
    ...
    "docs/fbt-rt-nextlat-research-plan-v4.md", "tests/test_olmo_f3e_retain.py",
...
OPTIONAL_PROJECT_FILES = {"docs/olmo1b-f3e-usage.md"}
...
REPORT_FILES = {"protocol.md", "results.md", "assessment.md", "test-results.txt", "summary.json",
    ...
    "retention-tests.txt", "environment.json", "profile-summary.json", "core-test-results.txt", "validator-test-results.txt", "reporting-test-results.txt", "execution-plan.json", "final-inputs.json"}
...
def source_name(name):
    ...
    require(not path.is_absolute() and path.parts and ".." not in path.parts
        ...
        and (name.startswith("cdrm/pretrained/") or name.startswith("scripts/")
             or name == "docs/reports/olmo1b-f3e/protocol.md"), "Unsafe/unscoped runtime source name")


def _checks(report, schema, failed):
    from scripts.olmo_f3e_report import declared_checks, configuration_check, validate_success
...
def validate_runs(runtime_dirs, *, project_root=ROOT, checkpoint_receipt=CHECKPOINT_RECEIPT,
    ...
    for value in runtime_dirs:
        ...
        require(schema in ESSENTIAL_SOURCES and report.get("finished_utc")
                and (status == "passed" or failed), "Require a completed supported F3e report")
        ...
        if protocol is not None:
            ...
            choices = ["protocol.md", "source-snapshot/docs/reports/olmo1b-f3e/protocol.md"]
    ...
    require(runs, "At least one completed F3e report is required")
...
def collect_evidence(runs, current_sources, references, *, project_root=ROOT,
    ...
    for name in sorted(OPTIONAL_PROJECT_FILES):
        if (project_root/name).exists() or (project_root/name).is_symlink():
            add(project_root, name, "project")
    for directory, pattern in (("scripts", "olmo_f3e_*.py"), ("tests", "test_olmo_f3e*.py")):
    ...
    for name in ("protocol.md", "results.md", "assessment.md", "test-results.txt"):
        require("report/"+name in members, f"Missing final F3e evidence: {name}")
    add(checkpoint_receipt.parent, checkpoint_receipt.name, "provenance")
    require(sum(path.stat().st_size for path in members.values()) <= MAX_EVIDENCE_BYTES, "F3e evidence exceeds small-archive budget")
...
def retain(args, *, project_root=ROOT):
    ...
    restore = ("# F3e native RT execution evidence\n\n"
        ...
        "F3e selects several native RT layers using the validated F3d bounded-workspace backward. "
        "All-sixteen-layer cases are stress diagnostics, not a chosen main architecture. "
        "Its primary gradient reference retains materialized backward on the same layer selection. "
...
def main(argv=None):
    ...
    parser.add_argument("--report-dir", type=Path, default=ROOT/"docs/reports/olmo1b-f3e")
