#!/usr/bin/env python3
"""CPU reports of saved conservative-input evaluations; no model inference.
...
LINEAGE = ROOT / ".runtime/rt-a5/20260915T163300Z-input-conservative10k"
SCHEMA = "rt-a5-conservative-input-report-v1"
TRAIN_SCHEMA = "rt-a5-conservative-input-training-v1"
SOURCE_SHA = "7e9da2bf91ee61991ee3b05d0f81287db13d72199fa74898c8a1dde78413804d"
REPORTING_SOURCES = ("scripts/rt_a5_conservative_input_report.py", "scripts/rt_a5_nextlat_report.py",
...
def coefficient(update):
    return .001 * min(max(update, 0) / 50000, 1) ** 2
...
def validate_endpoint(report, protocol, through_update):
    require(report.get("schema") == TRAIN_SCHEMA and report.get("start_update") == 0
            ...
            "Require the fresh conservative-input lineage")
...
def ten_k_comparison(capture, current, current_curves, order_reference):
    """Read only saved matched10k evaluations; never select an older run's best checkpoint."""
    rows = []
    for name, label, relative, training_name, expected_schema, value in (
        ("fixed", "Fixed λ=0.02", "20260915T150000Z-embedding-input10k", "train-injection",
         "rt-a5-embedding-injection-training-v1", .02),
        ("previous_quadratic", "Quadratic maximum 0.05 over 50k", "20260915T160000Z-input-quadratic50k", "train-quadratic",
         "rt-a5-quadratic-input-training-v1", .05 * (.2 ** 2)),
    ):
        lineage = ROOT / ".runtime/rt-a5" / relative
        protocol = json.loads(capture(f"comparison_{name}_protocol", lineage / "protocol.json"))
        directory = lineage / training_name
        require(local_path(protocol["training_directory"]).resolve() == directory, "Comparison training directory differs")
        old = json.loads(capture(f"comparison_{name}_report", directory / "report.json"))
        require(old["schema"] == expected_schema and old["status"] in ("complete", "stopped")
                and old["wandb"]["status"] == "synced" and old["completed_updates"] >= 10000
                and old["contract"] == protocol["strict_contract"] and old["initialization"] == protocol["initialization"],
                "Comparison needs a saved successful/stopped run containing10k")
        for key in ("model_config", "batch_size", "length", "train_rows", "data_manifest_sha256", "data_order_seed",
            ...
            require(old["contract"][key] == current["contract"][key], "Matched10k comparison contract differs")
        require(old["initialization"]["model_parameter_sha256"] == current["initialization"]["model_parameter_sha256"],
                "Matched10k comparison must share every initial learned tensor")
        if name == "fixed":
            require(protocol["coefficient"] == .02, "Fixed reference coefficient differs")
        else:
            require(protocol["maximum_coefficient"] == .05 and protocol["warmup_updates"] == 50000,
                    "Previous quadratic reference schedule differs")
        checkpoints = [c for c in old["checkpoints"] if c["completed_updates"] == 10000]
        require(len(checkpoints) == 1, "Comparison must have exactly one retained10k checkpoint")
        checkpoint = bound(checkpoints[0], directory / "checkpoints/step-010000.pt")
        history = [json.loads(line) for line in capture(f"comparison_{name}_history", directory / "history.jsonl").splitlines()]
        require(len(history) >= 10000 and all(
            row["update"] == original["update"] == update and row["order_chain"] == original["order_chain"]
            for update, (row, original) in enumerate(zip(history[:10000], order_reference[:10000]), 1)),
            "Matched10k comparison minibatch order differs")
        evaluated = [e for e in old["evaluations"] if e["update"] == 10000 and e["role"] == "ood_dev"]
        require(len(evaluated) == 1 and evaluated[0]["rows"] == 102400 and evaluated[0]["route"] == "backbone_only",
                "Comparison requires the same full length36 development pool at10k")
        if name == "previous_quadratic":
            require(evaluated[0]["injection_coefficient"] == value, "Comparison evaluation coefficient differs")
        endpoint = metric_rows(evaluated[0], name)[-1]
        rows.append({"arm": name, "label": label, "update": 10000, "injection_coefficient": value,
            "E36": endpoint["E"], "A36": endpoint["A"], "M36": endpoint["M"],
            "checkpoint": checkpoint, "training_wandb": old["wandb"]["run_url"]})
    endpoint = current_curves["10000"]["ood_dev"][-1]
    rows.append({"arm": "conservative", "label": "Quadratic maximum 0.001 over 50k", "update": 10000,
        "injection_coefficient": coefficient(10000), "E36": endpoint["E"], "A36": endpoint["A"], "M36": endpoint["M"],
        "training_wandb": current["wandb"]["run_url"]})
    return rows
...
def load_evidence(lineage=LINEAGE, through_update=None):
    ...
    require(protocol.get("schema") == "rt-a5-input-conservative-protocol-v1"
            ...
            and protocol["initial_coefficient"] == 0 and protocol["maximum_coefficient"] == .001
    ...
    require(directory == lineage / "train-conservative", "Training directory differs")
    ...
    require(schedule == report["schedule"] == initial["schedule"]
            and schedule["maximum_coefficient"] == .001 and schedule["warmup_updates"] == 50000
    ...
    for evaluation in report["evaluations"]:
        ...
        converted = metric_rows(evaluation, "conservative_input")
    ...
    if not partial:
        ...
        require(state.get("schema") == "rt-a5-conservative-input-final-state-validation-v1" and state.get("passed") is True
    ...
    comparisons = ten_k_comparison(capture, report, curves, reference) if end >= 10000 else []
    summary = {"schema": SCHEMA, "report_scope": "partial retained checkpoint" if partial else "terminal saved endpoint",
        ...
        "matched_ten_k_comparison": comparisons,
...
def plots(summary, output):
    ...
    for name, limits in (("length-full", (1, 36)), ("length-boundary", (10, 18))):
        ...
        fig.suptitle(f"Conservative input injection · {summary['report_scope']} through {summary['through_update']:,} · 102,400 words", y=.97)
        save(fig, name)
    ...
    axes[1].set(xlabel="Completed training updates", ylabel="Scheduled coefficient λ", title="λ(s) = 0.001 × min(s / 50,000, 1)²")
...
def markdown(summary):
    ...
    lines = [f"# Conservative input injection through {end:,} updates", "",
        f"{summary['report_scope'].capitalize()}; λ={coefficient(end):.6g}. Pilot endpoint: 10,000 updates; warmup spans 50,000 updates and remains incomplete at this pause. "
    ...
    if summary.get("matched_ten_k_comparison"):
        lines += ["", "Saved comparison at the same 10,000 updates, identical initial learned tensors and minibatch order:", "",
                  "| Input coefficient recipe | λ at 10k | E(36) | A(36) | M(36) |",
                  "|---|---:|---:|---:|---:|"]
        for row in summary["matched_ten_k_comparison"]:
            lines.append(f"| {row['label']} | {row['injection_coefficient']:.6g} | {row['E36']:.4%} | {row['A36']:.4%} | {row['M36']:.4%} |")
        lines += ["", "All three comparisons use the same repeatedly inspected 102,400-word development pool and one training seed. "
                  "The later stopping points of the earlier runs do not change these retained 10k results."]
...
def run(args):
    ...
    tracker = OnlineTracker(project="rt-a5-state-tracking", entity="taylorbollman", output_dir=output,
        ...
        name=f"conservative-input-report-through-{summary['through_update']:06d}")
