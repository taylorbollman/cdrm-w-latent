#!/usr/bin/env python3
"""Retain selected CE-integration evidence; reuse the native checkpoint by reference."""
...
SCHEMA = "olmo-ce-integration-retention-v1"
REPORT_SCHEMA = "olmo-ce-integration-v1"
MAX_BYTES = 64 * 1024**2
RUNTIME_RELATIVE = ".runtime/olmo-ce-integration"
DOCS_RELATIVE = "docs/reports/olmo-ce-integration"
...
PROJECT_FILES = (
    "scripts/olmo_ce_retain.py", "scripts/openelm_retain.py", "scripts/olmo_tiled_retain.py",
    "scripts/docker_shell.sh", "docker/requirements-docker.txt",
    "tests/test_nextlat_ce_chunk.py", "tests/test_olmo_nextlat.py",
    "tests/test_static_nextlat.py", "tests/test_static_training.py", "tests/test_nextlat_memory.py",
...
def collect_evidence(root=ROOT):
    ...
    members, names, source_differences = {}, set(), {}
    ...
    for row in summary["runs"]:
        ...
        require(isinstance(hashes, dict) and {
            "cdrm/pretrained/nextlat.py", "cdrm/pretrained/static_nextlat.py",
            "scripts/olmo_ce_integration.py",
        } <= hashes.keys(), "Missing essential runtime sources")
        differences = {}
    ...
    require(sum(p.stat().st_size for p in members.values()) < MAX_BYTES,
            "Evidence exceeds the 64 MiB uncompressed budget")
...
def main():
    ...
    prefix = f"cdrm-w-latent/fbt-rt-nextlat/olmo-ce-integration/{stamp}"
    archive = out / "evidence.tar.gz"
    inventory = build_evidence_archive(archive, [(p, n) for n, p in sorted(members.items())],
        "# CE integration and native OLMo evidence\n\n"
        ...
        "Failed diagnostics remain failed in the selection. These bounded integration/performance "
