#!/usr/bin/env python3
"""Deeper first-window RT diagnostic with the unchanged NextLat update math.

Fresh requested-depth Mitchell initialization; same data order, optimizer,
...
SCHEMA = "rt-a5-l1r-depth-training-v1"
VARIANT_ARCHITECTURES = {"rt_l1r_depth": "rt"}


def source_manifest():
    """Preserve all55 depth/order sources and add the requested-depth factory/driver/config."""
    sources = dict(depth_source_manifest())
    additions = [ROOT / "scripts" / name for name in
                 ("rt_a5_l1r_depth.py", "rt_a5_l1r_depth_train.py")]
    additions += list((ROOT / "configs/rt_a5_l1r_depth").glob("*.json"))
...
def load_checkpoint(path, *, model, optimizer, contract):
    ...
    if packet.get("schema") != SCHEMA or packet.get("contract") != contract:
        raise ValueError("L1R-depth checkpoint source/data/model/runtime/objective contract differs")
    ...
    if packet.get("initialization") != json_value(model.nextlat_initialization):
        raise ValueError("L1R-depth initialization differs")
...
def run(args):
    if args.variant not in VARIANT_ARCHITECTURES:
        raise ValueError("Unsupported L1R-depth variant")
    ...
    report = {
        ...
        "checkpoint_selection": "fixed10k depth diagnostic; confirmation not evaluated",
...
def parser():
    ...
    p.add_argument("--variant", choices=tuple(VARIANT_ARCHITECTURES), default="rt_l1r_depth")
