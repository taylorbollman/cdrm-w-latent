#!/usr/bin/env python3
"""One matched O4 learning arm with boundary checkpoints and online metrics."""
...
from scripts.olmo_o4_common import (ARMS, PilotTracker, preserve_rng, retain_file, source_hashes, write_event)
from scripts.olmo_validation import require_container_gpu


def evaluation(model, corpus, configuration, mode, rows, *, documents=False):
    results = {}
    for split in ("dev", "retention_dev"):
        count = min(rows, corpus.split_sizes[split])
        batch_size = configuration["eval_batch_size"]
        batches = (corpus.batch(split, range(i, min(count, i + batch_size)), device="cuda")
                   for i in range(0, count, batch_size))
        results[split] = evaluate_batches(model, batches, mode=mode, precision=configuration["precision"],
                                         include_document_records=documents)
    return results


def catastrophic(current, initial, margin):
    # Both domains must deteriorate severely; normal domain/retention tradeoffs
    # and the expected temporary RT conversion cost do not trigger this gate.
    return all(current[key]["mean_nll"] > initial[key]["mean_nll"] + margin
               for key in ("dev", "retention_dev"))


def finite_optimizer(model, optimizer):
    return (all(bool(torch.isfinite(p).all()) for p in model.parameters()) and
            all(bool(torch.isfinite(v).all()) for s in optimizer.state.values()
...
def main():
    ...
    configuration = {**configuration, "arm": args.arm, "storage_prefix": args.storage_prefix.rstrip("/")}
    ...
    report = json.loads(report_path.read_text()) if args.resume else {
        "schema": "olmo-o4-arm-v1", "status": "running", "arm": args.arm,
        "started_utc": datetime.now(timezone.utc).isoformat(), "configuration": configuration,
        "source_fingerprint": source, "evaluations": [], "checkpoints": [], "resumptions": []}
    if report["configuration"] != configuration or report["source_fingerprint"] != source:
    ...
    use_rt, enabled = ARMS[args.arm]
    state = load_native_state_dict(args.artifacts)
    model = build_model(state, enabled=enabled, chunk_size=configuration["nextlat_config"]["vocab_chunk_size"],
                        backend=configuration["attention_backend"], attention_precision=configuration["attention_precision"])
    del state
    if model.config.to_dict() != configuration["nextlat_config"]:
        raise ValueError("NextLat resolved configuration differs")
    ...
    optimizer = build_adamw(model, lr=configuration["lr"], betas=tuple(configuration["betas"]),
                           eps=configuration["eps"], weight_decay=configuration["weight_decay"])
    schedule = configuration["schedule"]
    scheduler = build_warmup_scheduler(optimizer, warmup_updates=schedule["warmup_updates"])
    ...
    if args.resume:
        saved = next((r for r in report["checkpoints"] if Path(r["path"]).name == args.resume.name), None)
        if saved is None:
            raise ValueError("Resume checkpoint must have a recorded integrity receipt")
        restored = load_training_checkpoint(args.resume, model, optimizer, scheduler=scheduler,
            configuration=configuration, source_fingerprint=source, expected_sha256=saved["sha256"])
    ...
    tracker = PilotTracker(project="pretrained-fbt-rt-nextlat", output_dir=args.output_dir,
        name=f"olmo-1b-o4-{args.arm}", group="olmo1b-o4-code-pilot", preserve_state=preserve_rng)
    ...
    def checkpoint(reason):
        ...
        if prior is None:
            ...
            record = save_training_checkpoint(path, model, optimizer, scheduler=scheduler, counters=counters,
                data_cursor={"next_window": cursor, "bad_evals": bad_evals}, configuration=configuration,
            ...
            record = prior
        record["storage"] = retain_file(path, f"{configuration['storage_prefix']}/{args.arm}/{name}",
    ...
    def evaluate(rows, *, full=False):
        alpha = alpha_for_update(schedule, counters.optimizer_updates, counters.input_tokens) if use_rt else 0.0
        mode = RTMode((0,), alpha) if use_rt else RTMode((), 0.0)
        ...
        event = {"kind": "evaluation", "update": counters.optimizer_updates, "input_tokens": counters.input_tokens,
                 "alpha": alpha, "full": full, "rows_requested": rows, "metrics": values}
        ...
        for split, result in values.items():
            for key in ("mean_nll", "perplexity", "next_token_accuracy", "ce_count"):
                metrics[f"{prefix}/{split}/{key}"] = result[key]
        tracker.log(metrics)
        print({"eval_update": counters.optimizer_updates, "tokens": counters.input_tokens, "alpha": alpha,
               "full": full, "code_nll": values["dev"]["mean_nll"],
               "retention_nll": values["retention_dev"]["mean_nll"]}, flush=True)
        persist()
        return values
    ...
    try:
        ...
        while counters.optimizer_updates < schedule["total_updates"]:
            ...
            result = optimizer_step(model, optimizer, [batch], config=training_config,
            ...
            event = {"kind": "update", "alpha": alpha, "seconds": seconds,
            ...
            metrics = {"update": counters.optimizer_updates, "train/input_tokens": counters.input_tokens,
                "train/supervised_tokens": counters.ce_positions, "train/alpha": alpha,
            ...
            if counters.optimizer_updates % 10 == 0:
                print({"update": counters.optimizer_updates, "tokens": counters.input_tokens, "alpha": alpha,
