#!/usr/bin/env python3
"""Retain bounded F3d evidence with exact per-run historical source snapshots."""
...
SCHEMA = "olmo-f3d-retention-v1"
PREFIX_ROOT = "gs://fast-chunks/cdrm-w-latent/fbt-rt-nextlat/olmo1b-f3d-rt-memory/"
NATIVE, TILE = ("olmo-f3d-native-v1", "olmo-f3d-probe-v1")
ESSENTIAL_SOURCES = {
    NATIVE: {"scripts/olmo_f3d_validate.py", "cdrm/pretrained/olmo_tiled.py", "cdrm/pretrained/olmo_rt_kernels.py", "cdrm/pretrained/olmo_rt_backward_kernels.py", "cdrm/pretrained/olmo_rt_memory.py", "cdrm/pretrained/olmo_rt_recompute_kernels.py"},
    TILE: {"scripts/olmo_f3d_probe.py", "cdrm/pretrained/olmo_tiled.py", "cdrm/pretrained/olmo_rt_kernels.py", "cdrm/pretrained/olmo_rt_backward_kernels.py", "cdrm/pretrained/olmo_rt_memory.py", "cdrm/pretrained/olmo_rt_recompute_kernels.py"},
}
EXTRA_PROJECT_FILES = {
    "cdrm/__init__.py", "cdrm/pretrained/__init__.py", "cdrm/pretrained/resource_estimates.py",
    "scripts/olmo_f3d_retain.py", "scripts/olmo_f3b_retain.py", "scripts/olmo_f3_retain.py", "scripts/olmo_f2_retain.py",
    ...
    "docs/fbt-rt-nextlat-research-plan-v4.md", "tests/test_olmo_f3d_retain.py",
...
OPTIONAL_PROJECT_FILES = {"docs/olmo1b-f3d-usage.md"}
...
def source_name(name):
    ...
    require(not path.is_absolute() and path.parts and ".." not in path.parts
        ...
        and (name.startswith("cdrm/pretrained/") or name.startswith("scripts/")
             or name == "docs/reports/olmo1b-f3d/protocol.md"), "Unsafe/unscoped runtime source name")
...
def _checks(report, schema, failed):
    ...
    if failed:
        ...
        if schema == NATIVE and report.get("stage") == "correctness":
            ...
            require((variant == "recompute" and expected_calls > 0)
...
def validate_runs(runtime_dirs, *, project_root=ROOT, checkpoint_receipt=CHECKPOINT_RECEIPT,
    ...
    for value in runtime_dirs:
        ...
        require(schema in ESSENTIAL_SOURCES and report.get("finished_utc")
                and (status == "passed" or failed), "Require a completed supported F3d report")
        ...
        if protocol is not None:
            ...
            choices = ["protocol.md", "source-snapshot/docs/reports/olmo1b-f3d/protocol.md"]
    ...
    require(runs, "At least one completed F3d report is required")
...
def collect_evidence(runs, current_sources, references, *, project_root=ROOT,
    ...
    for name in sorted(OPTIONAL_PROJECT_FILES):
        if (project_root/name).exists() or (project_root/name).is_symlink():
            add(project_root, name, "project")
    for directory, pattern in (("scripts", "olmo_f3d_*.py"), ("tests", "test_olmo_f3d*.py")):
    ...
    for name in ("protocol.md", "results.md", "assessment.md", "test-results.txt"):
        require("report/"+name in members, f"Missing final F3d evidence: {name}")
    add(checkpoint_receipt.parent, checkpoint_receipt.name, "provenance")
    require(sum(path.stat().st_size for path in members.values()) <= MAX_EVIDENCE_BYTES, "F3d evidence exceeds small-archive budget")
...
def retain(args, *, project_root=ROOT):
    ...
    restore = ("# F3d native RT execution evidence\n\n"
        ...
        "Synthetic fixtures do not consume the native checkpoint. F3d recomputes probabilities "
        "inside bounded backward tiles; sequential local VJPs remain. The primary baseline is F3c, "
        "with fused forward/backward tiles, cast reuse and materialized probability/error matrices. "
...
def main(argv=None):
    ...
    parser.add_argument("--report-dir", type=Path, default=ROOT/"docs/reports/olmo1b-f3d")
