"""F3c archival lineage, explicit diagnostic failures and create-only transport."""
import base64
import copy
import hashlib
import json
...
from scripts import olmo_f3c_retain as retain
...
def evidence(tmp_path):
    project, docs, directory = (tmp_path/name for name in ("project", "docs", "f3c-native-01"))
    ...
    report = {"schema": retain.NATIVE, "status": "passed", "stage": "correctness", "finished_utc": "now",
        "configuration": {"case": "combined", "stage": "correctness", "variant": "triton"},
        "checks": [{"name": "same_state_variant_vs_reference", "passed": True,
                    "forward_losses_bitwise_equal": True, "dispatch_matches": True,
                    "reference_fused_backward_calls": 0, "candidate_fused_backward_calls": 511,
                    "expected_candidate_fused_backward_calls": 511,
                    "strict_comparison_diagnostic": {"passed": False}}],
...
@pytest.mark.parametrize("damage", ["forward_loss", "dispatch_flag", "missing_calls", "count_mismatch",
    "reference_dispatched", "boolean_count", "no_fusion"])
def test_native_passing_gate_rejects_contradictory_loss_or_dispatch_evidence(evidence, damage):
    check = evidence.report["checks"][0]
    if damage == "forward_loss": check["forward_losses_bitwise_equal"] = False
    elif damage == "dispatch_flag": check["dispatch_matches"] = False
    elif damage == "missing_calls": check.pop("expected_candidate_fused_backward_calls")
    elif damage == "count_mismatch": check["candidate_fused_backward_calls"] = 510
    elif damage == "reference_dispatched": check["reference_fused_backward_calls"] = 1
    elif damage == "boolean_count": check["reference_fused_backward_calls"] = False
    elif damage == "no_fusion":
        check["candidate_fused_backward_calls"] = check["expected_candidate_fused_backward_calls"] = 0
    assert check["passed"] is True
    write(evidence.directory, "report.json", evidence.report)
    with pytest.raises(ValueError, match="Native correctness"):
        validate(evidence)
...
def test_successful_earlier_source_and_protocol_are_valid_historical_lineage(evidence):
    ...
    other = evidence.directory.with_name("f3c-native-02")
...
def set_profile_fields(report):
    counts = {"RT/local_writer_vjp": 512, "RT/local_finish_vjp": 512,
              "RT/batched_parameter_vjp": 1, "RT/attention_reconstruction": 1,
              "RT/reverse_historical_tile": 511, "RT/final_attention_matmul": 4}
    report.update(health={"passed": True}, annotated_eager_profile=[{"count": 1}],
        graph_profile=[{"count": 1}], annotation_calls=counts,
        completed_optimizer_updates=6,
        records=[{"counters": {"optimizer_updates": value}} for value in (4, 5, 6)],
        full_step={"median_wall_seconds": 3.1}, checks=[
            {"name": "observer_neutrality", "passed": True, "ownership_matches": True,
             "loss_keys_match": True, "nonidentical_losses": [], "nonidentical_gradients": []},
            {"name": "expected_backward_annotations", "passed": True, "expected": counts.copy(), "actual": counts.copy()},
            {"name": "finite_complete_updates", "passed": True}])


@pytest.mark.parametrize("schema", [retain.PROFILE, retain.TILE])
def test_distinct_diagnostic_scopes_are_preserved(evidence, schema):
    r = evidence.report
    r["schema"] = schema
    r["source_hashes"] = {name: retain.file_digest(evidence.project/name)["sha256"] for name in retain.ESSENTIAL_SOURCES[schema]}
    for name in r["source_hashes"]:
        write(evidence.directory, "source-snapshot/"+name, (evidence.project/name).read_text())
    if schema == retain.TILE:
        r.pop("checkpoint")
        (evidence.directory/"protocol.md").unlink()
        write(evidence.directory, "source-snapshot/docs/reports/olmo1b-f3c/protocol.md", (evidence.docs/"protocol.md").read_text())
        r["fixtures"] = {"path": "fixtures.pt", "sha256": "b"*64, "case_count": 60}
    if schema == retain.PROFILE:
        set_profile_fields(r)
    write(evidence.directory, "report.json", r)
    runs, _, _ = validate(evidence)
    assert runs[0]["provenance"]["uses_native_checkpoint"] is (schema == retain.PROFILE)
    assert runs[0]["provenance"]["reported_protocol_sha256"] == r["protocol_sha256"]
    if schema == retain.TILE:
        assert runs[0]["provenance"]["excluded_fixtures"] == r["fixtures"]
    else:
        assert runs[0]["provenance"]["completed_optimizer_updates"] == 6


@pytest.mark.parametrize("damage", ["missing_check", "failed_observer", "ownership", "changed_gradient",
    "call_count", "missing_trace", "invalid_timing", "update_total", "update_records", "missing_protocol"])
def test_passing_profile_cannot_hide_observer_or_completion_failure(evidence, damage):
    r = evidence.report
    r["schema"] = retain.PROFILE
    r["source_hashes"] = {name: retain.file_digest(evidence.project/name)["sha256"] for name in retain.ESSENTIAL_SOURCES[retain.PROFILE]}
    for name in r["source_hashes"]:
        write(evidence.directory, "source-snapshot/"+name, (evidence.project/name).read_text())
    set_profile_fields(r)
    if damage == "missing_check": r["checks"].pop(0)
    elif damage == "failed_observer": r["checks"][0]["passed"] = False
    elif damage == "ownership": r["checks"][0]["ownership_matches"] = False
    elif damage == "changed_gradient": r["checks"][0]["nonidentical_gradients"] = ["weight"]
    elif damage == "call_count": r["annotation_calls"]["RT/local_writer_vjp"] = 511
    elif damage == "missing_trace": r["graph_profile"] = []
    elif damage == "invalid_timing": r["full_step"]["median_wall_seconds"] = 0
    elif damage == "update_total": r["completed_optimizer_updates"] = 5
    elif damage == "update_records": r["records"][-1]["counters"]["optimizer_updates"] = 5
    elif damage == "missing_protocol": r.pop("protocol_sha256")
    write(evidence.directory, "report.json", r)
    with pytest.raises(ValueError):
        validate(evidence)


def test_failed_profile_retains_partial_progress_without_claiming_six_completed_updates(evidence):
    r = evidence.report
    r.update(schema=retain.PROFILE, status="failed", error_type="RuntimeError",
             error_message="diagnostic interrupted", completed_optimizer_updates=2)
    r["source_hashes"] = {name: retain.file_digest(evidence.project/name)["sha256"] for name in retain.ESSENTIAL_SOURCES[retain.PROFILE]}
    for name in r["source_hashes"]:
        write(evidence.directory, "source-snapshot/"+name, (evidence.project/name).read_text())
    r["wandb"]["status"] = "synced_failed_experiment"
    write(evidence.directory, "report.json", r)
    runs, _, _ = validate(evidence, allow_failed_diagnostics=True)
    assert runs[0]["provenance"]["completed_optimizer_updates"] == 2
    assert not runs[0]["provenance"]["counts_as_success"]


def test_historical_f3b_report_is_reference_only_not_accepted_as_f3c_runtime(evidence):
    evidence.report["schema"] = "olmo-f3b-native-v1"
    write(evidence.directory, "report.json", evidence.report)
    with pytest.raises(ValueError, match="supported F3c"):
        validate(evidence)
...
def test_archive_whitelist_excludes_models_fixtures_secrets_and_unrelated_logs(evidence):
    ...
    assert any(name.startswith("runtime/f3c-native-01/source-snapshot/") for _, name in members)
    ...
    with tarfile.open(archive) as tar:
        ...
        assert {"project/scripts/olmo_f3b_report.py", "project/scripts/olmo_f3_report.py"} <= set(tar.getnames())
