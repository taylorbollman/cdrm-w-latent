...
from scripts import olmo_f3c_validate as validation
...
def test_counter_observes_kernel_entry_without_any_cuda_execution(monkeypatch):
    ...
    def fake(*args, **kwargs):
        ...
        return sentinel
    monkeypatch.setattr(olmo_rt_backward_kernels, "backward_tile", fake)
    with validation.count_backward_tiles() as count:
        assert count == {"count": 0}
        assert olmo_rt_backward_kernels.backward_tile("metadata only") is sentinel
        assert olmo_rt_backward_kernels.backward_tile("second", flag=True) is sentinel
        assert count == {"count": 2}
    assert seen == [(("metadata only",), {}), (("second",), {"flag": True})]
    assert olmo_rt_backward_kernels.backward_tile is fake
...
def test_counter_restores_original_callable_even_when_kernel_raises(monkeypatch):
    ...
    with pytest.raises(DiagnosticFailure):
        with validation.count_backward_tiles() as count:
            olmo_rt_backward_kernels.backward_tile()
