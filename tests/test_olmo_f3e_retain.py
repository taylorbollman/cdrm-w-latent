"""F3e archival lineage, explicit diagnostic failures and create-only transport."""
import base64
import copy
import hashlib
import json
...
from scripts import olmo_f3e_retain as retain
from test_olmo_f3e_report import make_report
...
def evidence(tmp_path):
    project, docs, directory = (tmp_path/name for name in ("project", "docs", "f3e-native-01"))
    ...
    report = make_report(stage="correctness", layout="spread2")
...
def test_native_passing_gate_rejects_contradictory_loss_or_dispatch_evidence(evidence, damage):
    check = next(row for row in evidence.report["checks"] if row["name"] == "same_state_variant_vs_reference")
    if damage == "forward_loss": check["forward_losses_bitwise_equal"] = False
    elif damage == "dispatch_flag": check["dispatch_matches"] = False
    elif damage == "missing_calls": check.pop("expected_candidate_recompute_backward_calls")
    elif damage == "count_mismatch": check["candidate_recompute_backward_calls"] = 510
    elif damage == "reference_dispatched": check["reference_recompute_backward_calls"] = 1
    elif damage == "boolean_count": check["reference_recompute_backward_calls"] = False
    elif damage == "no_fusion":
        check["candidate_recompute_backward_calls"] = check["expected_candidate_recompute_backward_calls"] = 0
...
def test_successful_earlier_source_and_protocol_are_valid_historical_lineage(evidence):
    ...
    other = evidence.directory.with_name("f3e-native-02")
...
def test_historical_f3b_report_is_reference_only_not_accepted_as_f3e_runtime(evidence):
    evidence.report["schema"] = "olmo-f3b-native-v1"
    write(evidence.directory, "report.json", evidence.report)
    with pytest.raises(ValueError, match="supported F3e"):
        validate(evidence)
...
def test_archive_whitelist_excludes_models_fixtures_secrets_and_unrelated_logs(evidence):
    ...
    assert any(name.startswith("runtime/f3e-native-01/source-snapshot/") for _, name in members)
    ...
    with tarfile.open(archive) as tar:
        ...
        assert {"project/scripts/olmo_f3b_report.py", "project/scripts/olmo_f3_report.py", "project/scripts/olmo_f3e_report.py", "project/tests/test_olmo_multilayer_execution.py"} <= set(tar.getnames())
