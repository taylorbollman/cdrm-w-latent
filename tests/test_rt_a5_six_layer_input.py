...
from scripts import rt_a5_six_layer_input_train as driver
...
from scripts.rt_a5_six_layer_input import build_model, configuration, make_projection, SixLayerInjectedBackbone
...
def test_actual_width_six_layer_count_and_exact_shared_initialization():
    ...
    assert SixLayerInjectedBackbone.forward is EmbeddingInjectedBackbone.forward
...
def test_input_addition_is_only_before_block1_and_existing_normalization():
    ...
    expected = seen["first_output"] + .01 * model.backbone.embedding_projection(model.backbone.transformer.wte(x))
    assert torch.equal(seen["norm_inputs"][0], expected)
    assert all(not block._forward_pre_hooks for block in blocks)


def test_active_projection_and_original_embedding_gradients_final_latents_and_causality():
    model = small()
    x = inputs()
    seen = {}
...
def test_first_step_keeps_projection_attached_and_resume_is_exact(tmp_path):
    ...
    assert contract["injection_coefficient"] == .01 and contract["n_layers"] == 6
    ...
    assert restored.backbone.injection_coefficient == .01
    ...
    with pytest.raises(ValueError, match="fixed input coefficient"):
...
def test_driver_stops_after_exact_completed_update_or_completes_endpoint(tmp_path, monkeypatch, stop_at, checkpoints):
    ...
    def update(model, optimizer, tx, ty, **kwargs):
        step = len(calls) + 1
        assert model.backbone.injection_coefficient == .01
        calls.append(step)
    ...
    assert packet["injection_coefficient"] == .01
