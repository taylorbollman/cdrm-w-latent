"""F3d archival lineage, explicit diagnostic failures and create-only transport."""
import base64
import copy
import hashlib
import json
...
from scripts import olmo_f3d_retain as retain
...
def evidence(tmp_path):
    project, docs, directory = (tmp_path/name for name in ("project", "docs", "f3d-native-01"))
    ...
    report = {"schema": retain.NATIVE, "status": "passed", "stage": "correctness", "finished_utc": "now",
        "configuration": {"case": "combined", "stage": "correctness", "variant": "recompute"},
...
def test_successful_earlier_source_and_protocol_are_valid_historical_lineage(evidence):
    ...
    other = evidence.directory.with_name("f3d-native-02")
...
def test_historical_f3b_report_is_reference_only_not_accepted_as_f3d_runtime(evidence):
    evidence.report["schema"] = "olmo-f3b-native-v1"
    write(evidence.directory, "report.json", evidence.report)
    with pytest.raises(ValueError, match="supported F3d"):
        validate(evidence)
...
def test_archive_whitelist_excludes_models_fixtures_secrets_and_unrelated_logs(evidence):
    ...
    assert any(name.startswith("runtime/f3d-native-01/source-snapshot/") for _, name in members)
...
def test_probe_is_retained_without_claiming_native_checkpoint_consumption(evidence):
    r=evidence.report
    r["schema"]=retain.TILE
    r.pop("checkpoint")
    r["source_hashes"]={name:retain.file_digest(evidence.project/name)["sha256"] for name in retain.ESSENTIAL_SOURCES[retain.TILE]}
    for name in r["source_hashes"]:
        write(evidence.directory,"source-snapshot/"+name,(evidence.project/name).read_text())
    r["fixtures"]={"path":"fixtures.pt","sha256":"b"*64,"case_count":66}
    (evidence.directory/"protocol.md").unlink()
    write(evidence.directory,"source-snapshot/docs/reports/olmo1b-f3d/protocol.md",(evidence.docs/"protocol.md").read_text())
    write(evidence.directory,"report.json",r)
    runs,_,_=validate(evidence)
    assert not runs[0]["provenance"]["uses_native_checkpoint"]
    assert runs[0]["provenance"]["excluded_fixtures"]==r["fixtures"]
    r["checkpoint"]={"sha256":retain.CHECKPOINT_SHA256}
    write(evidence.directory,"report.json",r)
