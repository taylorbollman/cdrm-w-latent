"""F3b archival lineage, explicit diagnostic failures and create-only transport."""
import base64
import copy
import hashlib
import json
...
from scripts import olmo_f3b_retain as retain
...
def evidence(tmp_path):
    project, docs, directory = (tmp_path/name for name in ("project", "docs", "f3b-native-01"))
    ...
    report = {"schema": retain.NATIVE, "status": "passed", "stage": "correctness", "finished_utc": "now",
        ...
        "checks": [{"name": "declared_gate", "passed": True,
...
def test_successful_earlier_source_and_protocol_are_valid_historical_lineage(evidence):
    ...
    other = evidence.directory.with_name("f3b-native-02")
...
def test_failures_require_opt_in_and_never_count_as_success(evidence):
    ...
    report.update(status="failed", stage="complete", error_type="AssertionError", error_message="screen failed")
...
@pytest.mark.parametrize("schema", [retain.PROFILE, retain.TILE, retain.FA4])
def test_distinct_diagnostic_scopes_are_preserved(evidence, schema):
    ...
    if schema in (retain.TILE, retain.FA4):
        r.pop("checkpoint")
    if schema in (retain.PROFILE, retain.FA4):
        r.pop("protocol_sha256")
    if schema == retain.PROFILE:
        r.update(health={"passed": True}, annotated_eager_profile=[{"count": 1}], graph_profile=[{"count": 1}],
                 records=[{"update": 6}], full_step={"median_wall_seconds": 3.1})
    if schema == retain.FA4:
        r.update(script_sha256=r["source_hashes"]["scripts/olmo_fa4_smoke.py"],
            launcher_sha256=r["source_hashes"]["scripts/docker_shell.sh"], capture_output_exact=True,
            interface={"path": "/site-packages/flash_attn/cute/interface.py", "sha256": "b"*64},
            versions={"flash-attn-4": "4.0.0b20"},
            comparisons={name: {"relative_l2": .002, "max_relative": .003} for name in ("output", "dq", "dk", "dv")})
        r.pop("source_hashes"); r.pop("configuration")
    write(evidence.directory, "report.json", r)
    runs, _, _ = validate(evidence)
    assert runs[0]["provenance"]["uses_native_checkpoint"] is (schema == retain.PROFILE)
    if schema != retain.TILE:
        assert runs[0]["provenance"]["reported_protocol_sha256"] is None
    else:
...
def test_archive_whitelist_excludes_models_fixtures_secrets_and_unrelated_logs(evidence):
    ...
    assert any(name.startswith("runtime/f3b-native-01/source-snapshot/") for _, name in members)
