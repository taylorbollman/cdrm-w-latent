...
from scripts import olmo_ordinary_efficiency_retain as retain
from test_olmo_ordinary_efficiency_report import add_run, evidence, write
...
def test_mixed_revisions_failures_dependencies_and_exact_traces_are_retained(ready):
    ...
    run(ready, "fa4-profile", arm="fa4", profile=True)
    summary(ready)
    ...
    assert checked["physical_optimizer_updates"] == 8
    ...
    assert checked["dependency_files_checked"]["fa4-profile"] == 1
    assert checked["profile_count"] == 1
    assert "runtime/fa4-profile/operator-trace.json.gz" in members
    assert "runtime/fa4-profile/dependency-snapshot/flash_attn/cute/interface.py" in members
    assert "runtime/early-failure/report.json" in members
    assert "project/scripts/olmo_ordinary_efficiency_plot.py" in members


def test_completed_operational_diagnostic_retains_failed_numerics_and_six_updates(ready):
    path, raw = run(ready, "qualified", stage="correctness", arm="fa4")
    qualify(path, raw)
...
def test_changed_evidence_cannot_be_retained(ready, damage):
    path, raw = run(ready, "fa4-profile", arm="fa4", profile=True)
    selected = summary(ready)
    if damage == "summary_updates":
        ...
        write(path.parent, "dependency-snapshot/flash_attn/cute/interface.py", "different dependency")
    elif damage == "trace":
        write(path.parent, "operator-trace.json.gz", b"different trace")
    elif damage == "log":
        (path.parent.parent / "fa4-profile.log").unlink()
