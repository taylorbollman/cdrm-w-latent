"""Intentional CPU coverage of native OLMo's sequential RT contract."""
...
from cdrm.pretrained.olmo_recurrent import (
    OLMoRecurrentCache,
    OLMoRTForCausalLM,
    RTMode,
...
def fixture_model():
    return OLMoRTForCausalLM(OLMoConfig.tiny(), attention_backend="math")
...
def test_alpha_zero_matches_ordinary_outputs_inputs_and_all_parameter_gradients(selected_layers):
    model = fixture_model()
    ordinary = OLMoForCausalLM(model.config, attention_backend="math")
...
def test_persistent_cache_is_native_unrotated_memory_from_pre_finalnorm_output(alpha):
    ...
    _, key, value = layer.project_qkv(layer.attn_norm((1 - alpha) * x + alpha * z))
    torch.testing.assert_close(result.past_key_values.key_values[0][0], key, rtol=3e-6, atol=1e-6)
...
def test_checkpoint_layout_and_tied_ownership_are_unchanged():
    model = fixture_model()
    ordinary = OLMoForCausalLM(model.config)
...
def test_cache_rejects_changed_mode_model_or_weights():
    ...
    with torch.no_grad():
        model.layers[0].att_proj.weight.add_(0.1)
    with pytest.raises(ValueError, match="weights"):
