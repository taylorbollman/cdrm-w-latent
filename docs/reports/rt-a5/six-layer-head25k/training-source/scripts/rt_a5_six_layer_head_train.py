#!/usr/bin/env python3
"""Fresh six-layer reassigned embedding-access head, initial 10k diagnostic.
...
from scripts.rt_a5_six_layer_head import build_model
...
SCHEMA = "rt-a5-six-layer-head-training-v1"
VARIANT_ARCHITECTURES = {"embedding_head": "rt"}


def source_manifest():
    """Preserve the historical 62 sources and add this head adapter/factory/driver/config."""
    sources = dict(injection_source_manifest())
    additions = [ROOT / "scripts" / name for name in
                 ("rt_a5_embedding_head.py", "rt_a5_six_layer_head.py", "rt_a5_six_layer_head_train.py")]
    additions += list((ROOT / "configs/rt_a5_six_layer_head").glob("*.json"))
...
def validate_head_state(model):
    block = model.backbone.transformer.blocks[1]
    if (model.backbone.embedding_head_enabled is not True
            or model.backbone.config.n_layers != 6
            or block.embedding_head_index != model.backbone.config.n_heads - 1):
        raise ValueError("Require the enabled last-head reassignment in six-layer block1")
...
def save_checkpoint(path, *, model, optimizer, contract, completed, order_chain,
    ...
    return {"path": str(path.resolve()), "sha256": file_sha256(path),
            ...
            "examples_seen": packet["examples_seen"], "embedding_head_enabled": True}
...
def load_checkpoint(path, *, model, optimizer, contract):
    ...
    if packet.get("schema") != SCHEMA or packet.get("contract") != contract:
        raise ValueError("Six-layer head checkpoint source/data/model/runtime/objective contract differs")
    ...
    if packet.get("embedding_head_enabled") is not True:
        raise ValueError("Checkpoint embedding-head route differs")
    if packet.get("initialization") != json_value(model.nextlat_initialization):
        raise ValueError("Six-layer head initialization differs")
...
def make_contract(args, *, model, sources, data_root, train_x, hardware):
    ...
    contract.update(
        ...
        n_layers=6,
        embedding_head_enabled=True, embedding_head_layer=1,
        embedding_head_index=model.backbone.config.n_heads - 1,
...
def run(args):
    if args.variant not in VARIANT_ARCHITECTURES:
        raise ValueError("Unsupported Six-layer head variant")
    ...
    report = {
        ...
        "checkpoint_selection": "initial 10k diagnostic; one existing embedding-access head; user may stop after completed updates; confirmation not evaluated",
        "embedding_head_enabled": True,
    ...
    try:
        ...
        for update in range(completed + 1, args.updates + 1):
            ...
            row = {"update": update, "examples_seen": update * args.batch_size,
                   ...
                   "embedding_head_enabled": True, **values}
            history.write(json.dumps(row, allow_nan=False) + "\n")
            window.append(row)
            if should_log:
                logged = {"update": update, "train/embedding_head_enabled": 1,
            ...
            if update % args.eval_every == 0 or update in checkpoint_steps or stopping is not None:
                ...
                for role, (dx, dy) in dev.items():
                    ...
                    tracker.log({"update": update, "embedding_head/enabled": 1,
        ...
        tracker.summary({"completed_updates": completed, "requested_endpoint": args.updates,
                         ...
                         "embedding_head_enabled": True, "confirmation_evaluated": False,
...
def parser():
    ...
    p.add_argument("--variant", choices=tuple(VARIANT_ARCHITECTURES), default="embedding_head")
