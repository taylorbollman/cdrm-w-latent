...
class LRMonitor:
    ...
    def check(self) -> Dict[str, float]:
        ...
        lrs += [group["lr2"] for group in self.optim.param_groups if 'lr2' in group]
...
class Trainer:
    ...
    def train_step(self, batch: Dict[str, Any], reduce_global_loss: bool = True) -> Dict[str, float]:
        ...
        for group in self.optim.param_groups:
            ...
            if 'lr2' in group:
                group["lr2"] = self.scheduler.get_lr(
                self.cfg.optimizer.learning_rate2, self.scheduler_current, self.scheduler_max
            ) 
