#!/usr/bin/env python3
"""Combined RT initialization/position pilot using unchanged NextLat update math.
...
from scripts.rt_a5_nextlat_train import (
    ...
    source_manifest as nextlat_source_manifest, train_step,
)
...
SCHEMA = "rt-a5-nextlat-variant-training-v1"


def source_manifest():
    """Extend the complete frozen NextLat training identity with this variant."""
    sources = dict(nextlat_source_manifest())
    additions = [ROOT / "scripts" / name for name in
                 ("rt_a5_nextlat_variant.py", "rt_a5_nextlat_variant_train.py")]
    additions += list((ROOT / "configs/rt_a5_nextlat_variant").glob("*.json"))
...
def load_checkpoint(path, *, model, optimizer, contract):
    ...
    if packet.get("schema") != SCHEMA or packet.get("contract") != contract:
        raise ValueError("NextLat variant checkpoint source/data/model/runtime/objective contract differs")
    ...
    if packet.get("initialization") != json_value(model.nextlat_initialization):
        raise ValueError("NextLat variant initialization differs")
...
def make_contract(args, *, model, sources, data_root, train_x, hardware):
    ...
    contract.update(
        schema=SCHEMA,
        variant_config=json_value(model.variant_config),
...
def run(args):
    ...
    model = build_variant_model(args.architecture, **model_kwargs)
    ...
    tracker = OnlineTracker(
        ...
        name=args.wandb_run_name or f"{args.architecture}-nextlat-variant-d{args.width}-seed{args.seed}",
