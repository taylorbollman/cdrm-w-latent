#!/usr/bin/env python3
"""Fresh four-layer input bypass with quadratic lambda warmup through 50k.
...
SCHEMA = "rt-a5-quadratic-input-training-v1"
...
def source_manifest():
    ...
    additions = [ROOT / "scripts" / name for name in
                 ("rt_a5_quadratic_input.py", "rt_a5_quadratic_input_train.py")]
    additions += list((ROOT / "configs/rt_a5_quadratic_input").glob("*.json"))
...
def load_checkpoint(path, *, model, optimizer, contract):
    ...
    if packet.get("schema") != SCHEMA or packet.get("contract") != contract:
        raise ValueError("Quadratic-input checkpoint source/data/model/runtime/objective contract differs")
    ...
    if (packet.get("schedule") != SCHEDULE or packet.get("schedule_update") != completed
        ...
        raise ValueError("Checkpoint quadratic schedule or current coefficient differs")
    if packet.get("initialization") != json_value(model.nextlat_initialization):
        raise ValueError("Quadratic-input initialization differs")
...
def run(args):
    if args.variant not in VARIANT_ARCHITECTURES:
        raise ValueError("Unsupported Quadratic-input variant")
    ...
    report = {
        ...
        "checkpoint_selection": "planned 50k quadratic warmup; user may stop after completed updates; confirmation not evaluated",
