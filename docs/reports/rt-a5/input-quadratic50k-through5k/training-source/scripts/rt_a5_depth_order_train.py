#!/usr/bin/env python3
"""Four-layer SEQ and first-window RT pilots using unchanged NextLat update math.
...
from scripts.rt_a5_window_train import source_manifest as window_source_manifest
...
SCHEMA = "rt-a5-depth-order-training-v1"
VARIANT_ARCHITECTURES = {"seq4_alibi": "seq", "rt_window2_first": "rt"}


def source_manifest():
    """Preserve the 52-source window lineage and cover the new depth/order pair."""
    sources = dict(window_source_manifest())
    additions = [ROOT / "scripts" / name for name in
                 ("rt_a5_depth_order.py", "rt_a5_depth_order_train.py")]
    additions += list((ROOT / "configs/rt_a5_depth_order").glob("*.json"))
...
def load_checkpoint(path, *, model, optimizer, contract):
    ...
    if packet.get("schema") != SCHEMA or packet.get("contract") != contract:
        raise ValueError("Depth/order checkpoint source/data/model/runtime/objective contract differs")
    ...
    if packet.get("initialization") != json_value(model.nextlat_initialization):
        raise ValueError("Depth/order initialization differs")
...
def run(args):
    if args.variant not in VARIANT_ARCHITECTURES:
        raise ValueError("Unsupported depth/order variant")
