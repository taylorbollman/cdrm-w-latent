...
def make_summary(run_dir, architecture, prefix_report=None, prefix_assessment=None):
    ...
    if prefix_report is not None:
        ...
        if (set(check.get("checks", {})) != expected_lengths
                or any(c.get("prediction_disagreements") != 0 or c.get("accuracy_counts_equal") is not True
            ...
            raise ValueError("Prefix check has missing or unequal predictions/counts, or modified model state")
        strict_passed = (check.get("status") == "complete" and check.get("passed") is True
                         and all(c.get("passed") is True for c in check["checks"].values()))
...
def markdown_report(summary):
    ...
    if summary["prefix_check"]:
        lines += ["The new forward-only truncation check uses **1,024 words**, distinct from the **102,400-word "
                  "historical curves** above. All checked shapes produced identical predicted states and E/A/M counts, "
                  "with unchanged weights and no gradients or optimizer updates. "
