#!/usr/bin/env python3
"""Mitchell RT positional/window pilots using unchanged NextLat update math.

This driver has an independent source and checkpoint identity. It imports the
original joint update, one-step diagnostics, optimizer and backbone evaluator
...
from scripts.rt_a5_nextlat_variant_train import source_manifest as variant_source_manifest
...
def source_manifest():
    """Cover reused sinusoidal code plus the new model, driver and configs."""
    sources = dict(variant_source_manifest())
    additions = [ROOT / "scripts" / name for name in
                 ("rt_a5_window.py", "rt_a5_window_train.py")]
    additions += list((ROOT / "configs/rt_a5_window").glob("*.json"))
...
def load_checkpoint(path, *, model, optimizer, contract):
    ...
    if packet.get("schema") != SCHEMA or packet.get("contract") != contract:
        raise ValueError("RT window checkpoint source/data/model/runtime/objective contract differs")
    ...
    if packet.get("initialization") != json_value(model.nextlat_initialization):
        raise ValueError("RT window initialization differs")
...
def make_contract(args, *, model, sources, data_root, train_x, hardware):
    contract = baseline_make_contract(
        args, model=model, sources=sources, data_root=data_root,
...
def run(args):
    ...
    tracker = OnlineTracker(
        ...
        name=args.wandb_run_name or (
            f"{args.architecture}-nextlat-{args.position_encoding}"
            f"-window{args.second_layer_window or 'full'}-d{args.width}-seed{args.seed}"),
...
def parser():
    ...
    p.add_argument("--architecture", choices=("rt",), default="rt")
    p.add_argument("--position-encoding", choices=("alibi", "sinusoidal"), default="sinusoidal")
    p.add_argument("--second-layer-window", type=int, choices=(2,), default=None,
                   help="Restrict only RT layer 2 to temporary self plus the preceding permanent state")
